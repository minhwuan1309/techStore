...
const MemberSidebar = ({ isMobile = false, onClose }) => {
  ...
  return (
    <div className={`${isMobile ? 'w-full' : 'bg-white h-screen py-4 w-[250px] flex-none rounded-lg shadow-lg m-4 mr-0'} flex flex-col justify-between text-lg`}>
      <div className="w-full flex flex-col items-center justify-center py-4 md:py-6 border-b border-gray-200">
        <img
          src={current?.avatar || avatar}
          alt="avatar"
          className="w-16 h-16 md:w-24 md:h-24 object-cover rounded-full border-4 border-indigo-500 mb-3 md:mb-4 hover:scale-105 transition-transform duration-300"
        />
        <h3 className="text-lg md:text-xl font-semibold text-gray-800">{`${current?.firstname} ${current?.lastname}`}</h3>
        <p className="text-xs md:text-sm text-gray-500">Thành viên</p>
      </div>
      ...
      <NavLink 
        to={"/"} 
        onClick={handleNavClick}
        className={clsx(notActivedStyle, "mb-4 md:mb-8 border-t border-gray-200 pt-4 mx-2")}
      >
